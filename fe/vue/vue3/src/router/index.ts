...
// Fungsi untuk memeriksa autentikasi JWT
function isAuthenticated() {
    ...
    if (token) {
        try {
            ...
            // Jika token kadaluarsa, hapus dari localStorage
...
},
{
    ...
    //academic year
    {
        ...
        name: 'admin_academic_year',
        component: () => import(/* webpackChunkName: "analytics" */ '../views/admin/academic-year.vue'),
        meta: { requiresAuth: true }
    },
    //teacher
    {
        path: '/school/teacher',
        name: 'admin_teacher',
        component: () => import(/* webpackChunkName: "analytics" */ '../views/admin/teacher.vue'),
        meta: { requiresAuth: true }
    },
    //student
    {
...
router.beforeEach((to, from, next) => {
    ...
    if (to.matched.some((record) => record.meta.requiresAuth)) {
        if (isAuthenticated()) {
            ...
            next({ name: 'cover-login' }); // Token tidak valid, arahkan ke login
        }
    } else {
